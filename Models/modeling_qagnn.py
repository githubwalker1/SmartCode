from Models.modeling_encoder import TextEncoder, MODEL_NAME_TO_CLASS
...
参数类型：整数。
作用：用于指定图神经网络层 (self.gnn) 中的邻接矩阵传播步数（message passing steps）。
n_ntype:
...
参数类型：整数。
作用：指定概念的数量，用于嵌入层 (self.concept_emb) 的初始化。
concept_dim:

参数类型：整数。
作用：指定概念嵌入的维度，也是图神经网络层 (self.gnn) 输入和输出的维度。
concept_in_dim:

参数类型：整数。
作用：指定概念嵌入的输入维度，可能是预训练概念嵌入的维度。
n_attention_head:

参数类型：整数。
作用：指定多头自注意力池化层 (self.pooler) 中的注意力头数。
fc_dim:

参数类型：整数。
作用：指定全连接层MLP (self.fc) 的隐藏层维度。
n_fc_layer:

参数类型：整数。
作用：指定全连接层 (self.fc) 的层数。
p_emb, p_gnn, p_fc:

参数类型：浮点数。
作用：分别指定嵌入层 (self.concept_emb)、图神经网络层 (self.gnn) 和全连接层 (self.fc) 的 dropout 概率。
pretrained_concept_emb:

参数类型：Tensor 或者 None。
作用：如果提供了预训练的概念嵌入，则用于初始化嵌入层 (self.concept_emb)。
freeze_ent_emb:
...
参数类型：浮点数。
作用：用于初始化权重的范围，通过调用 _init_weights 方法来实现。
'''
...
class QAGNN(nn.Module):
    ...
    '''
    sent_vecs: 形状为 (batch_size, dim_sent) 的张量，表示每个样本的句子向量。
    concept_ids: 形状为 (batch_size, n_node) 的张量，表示每个样本中的节点的概念标识。
    node_type_ids: 形状为 (batch_size, n_node) 的张量，表示每个节点的类型。
                    其中，0 表示问题实体（question entity），1 表示答案选择实体（answer choice entity），
                    2 表示其他节点，3 表示上下文节点。
    node_scores: 形状为 (batch_size, n_node, 1) 的张量，表示每个节点的分数。
    adj_lengths: 形状为 (batch_size,) 的张量，表示每个样本中节点的数量。
    adj: 表示图的边，包括 edge_index 和 edge_type。
    emb_data: （可选）嵌入数据。
    cache_output: 一个布尔值，指示是否缓存输出
    '''
...
class LM_QAGNN(nn.Module):
    def __init__(self, args, model_name, k, n_ntype, n_etype,
                 n_concept, concept_dim, concept_in_dim,
                 n_attention_head,
                 fc_dim, n_fc_layer, p_emb, p_gnn, p_fc,
                 pretrained_concept_emb=None, freeze_ent_emb=True,
...
class LM_QAGNN_DataLoader(object):
    '''
    args: 存储训练参数的对象。
    train_statement_path, dev_statement_path, test_statement_path: 训练、开发和测试数据的文件路径。
    train_adj_path, dev_adj_path, test_adj_path: 训练、开发和测试数据的邻接矩阵文件路径。
    batch_size: 训练时使用的批量大小。
    eval_batch_size: 评估时使用的批量大小。
    device: 模型运行的设备。
    model_name: 模型的名称。
    max_node_num: 图中节点的最大数量。
    max_seq_length: 序列的最大长度。
    is_inhouse: 是否为内部训练。
    inhouse_train_qids_path: 内部训练时使用的问题ID文件路径。
    subsample: 数据的子采样比例。
    use_cache: 是否使用缓存。
    '''
